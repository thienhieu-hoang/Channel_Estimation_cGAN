...
params.scene_last = 1;
params.maximumDopplerShift = 50; % Hz

% OFDM setting
params.OFDM.mu = 1;
if params.OFDM.mu == 1
    params.OFDM.NSizeGrid   = 51;     % 51 Resource Blocks
    params.OFDM.delta_f     = 30;     % kHz, banddwidth of 1 subcarrier
    params.OFDM.RB          = 12*params.OFDM.delta_f; % kHz, bandwidth of 1 RB
                                                      % total transmission bandwidth is 12*51*30
    params.OFDM.num_subc    = 12 * params.OFDM.NSizeGrid; % 612
    params.OFDM.num_symbol  = 14;     % number of OFDM symbols per slot
    params.OFDM.T_symbol    = 35.71 * 1e-6; % s, symbol duration
    params.OFDM.T_slot      = 500 * 1e-6;   % s, slot duration
    params.bandwidth        = 20e6;   % Hz, channel bandwidth
                                       % 20MHz
end
...
% Active users
params.active_user_first = 1; %545;        % The first row of the considered user section (check the scenario description for the user row map)
params.active_user_last  = 1; %600;        % The last row of the considered user section (check the scenario description for the user row map)
...
% OFDM parameters
params.num_OFDM = params.OFDM.num_subc; % 612          % Number of OFDM subcarriers
...
params.OFDM_limit = params.num_OFDM;               % Only the first params.OFDM_limit subcarriers will be considered  
